...
export default function FirmLogin() {
  ...
  useEffect(() => {
    ...
    const checkAuth = () => {
      ...
      try {
        ...
        console.error('[FirmLogin] Firebase auth error:', err);
        if (isMounted) {
          if (authTimeout) clearTimeout(authTimeout);
          setChecking(false);
        }
  ...
  async function handleLogin(e) {
    ...
    try {
      const rlRes = await fetch('/api/check-rate-limit', {
        ...
        body: JSON.stringify({ action: 'firm-login' }),
      });
  ...
  return (
    <>
      <Head>
        <title>Partner Portal — AMS Derive</title>
          ...
          <form onSubmit={handleLogin} className={styles.loginForm}>
            <div className={styles.fieldGroup}>
              ...
              <input
                ...
                placeholder="firm@firms.amsderive.in"
