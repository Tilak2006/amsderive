...
const RechartsComponents = dynamic(
  ...
  { ssr: false, loading: () => <div className={styles.tableLoading} style={{ minHeight: '400px' }}>Loading charts...</div> }
);
...
export default function SubadminAnalytics() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      if (u) { setUser(u); setChecking(false); }
      else router.replace('/subadmin/login');
    });
    return () => unsubscribe();
  }, [router]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const res = await fetch('/api/subadmin/export-registrants', {
        method: 'POST',
  ...
  const analytics = useMemo(() => {
    ...
    const today = registrants.filter((r) => r.submittedAt?.slice(0, 10) === todayStr).length;
    ...
    const dailyData = Object.entries(dayMap).map(([date, count]) => ({ date: formatDate(date + 'T00:00:00'), count }));

    const uniData = Object.entries(uniCounts).sort((a, b) => b[1] - a[1]).slice(0, 15).map(([name, count]) => ({ name, count }));
    const consentPie = [
  ...
  if (checking) {
    return <div className={styles.checkingWrap}><span className={styles.checkingDot} /></div>;
  }

  return (
    <>
      <Head>
        <title>Analytics — AMS Derive Team</title>
      ...
      <div className={styles.dashPage}>
        ...
        <main className={styles.dashMain}>
          <div className={styles.tabBar}>
            <Link href="/subadmin/dashboard" className={styles.tab}>REGISTRANTS</Link>
                  ...
                  <div className={styles.chartsGrid}>
                      ...
                      <div className={styles.chartWrap}>
                        <ResponsiveContainer width="100%" height={300}>
                          <BarChart data={analytics.dailyData} margin={{ top: 8, right: 8, bottom: 0, left: -16 }}>
                            ...
                            <XAxis dataKey="date" tick={{ fill: LABEL_COLOR, fontSize: 10, fontFamily: 'var(--font-mono)' }} interval="preserveStartEnd" angle={-45} textAnchor="end" height={60} />
                            <YAxis tick={{ fill: LABEL_COLOR, fontSize: 11, fontFamily: 'var(--font-mono)' }} allowDecimals={false} />
                      ...
                      <div className={styles.chartWrap}>
                        <ResponsiveContainer width="100%" height={Math.max(300, analytics.uniData.length * 32)}>
                          <BarChart data={analytics.uniData} layout="vertical" margin={{ top: 8, right: 16, bottom: 0, left: 8 }}>
                            ...
                            <XAxis type="number" tick={{ fill: LABEL_COLOR, fontSize: 11, fontFamily: 'var(--font-mono)' }} allowDecimals={false} />
                            <YAxis type="category" dataKey="name" tick={{ fill: TEXT_COLOR, fontSize: 11, fontFamily: 'var(--font-mono)' }} width={160} />
                            <Tooltip content={<ChartTooltip />} cursor={{ fill: 'rgba(212,175,55,0.06)' }} />
                    ...
                    <div className={styles.chartCard}>
                      ...
                      <div className={styles.chartWrap} style={{ display: 'flex', justifyContent: 'center' }}>
                        <ResponsiveContainer width="100%" height={300}>
                          <PieChart>
                            <Pie data={analytics.consentPie} cx="50%" cy="50%" innerRadius={60} outerRadius={110} dataKey="value" stroke="#0e0e0e" strokeWidth={2} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
                              <Cell fill={GOLD} />
                              <Cell fill={DARK_RED} />
                            </Pie>
